...
export function SortableTable({ columns, rows }) {
  ...
  const visible = useMemo(() => {
    ...
    const sorted = matching
      ...
      .sort((a, b) => compareValues(a.row[sort.key], b.row[sort.key], column.type) || a.index - b.index)
      .map(({ row }) => row);
    return sort.dir === 'ascending' ? sorted : sorted.reverse();
  }, [rows, columns, sort, filter]);

  const toggle = (key) =>
    setSort((prev) => (prev?.key === key && prev.dir === 'ascending' ? { key, dir: 'descending' } : { key, dir: 'ascending' }));
