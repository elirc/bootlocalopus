...
export function createApp({ users }) {
  ...
  return http.createServer(async (req, res) => {
    ...
    if (req.method === 'PATCH') {
      ...
      if (ifMatch !== etag(order)) return fail(res, 412, 'PRECONDITION_FAILED');
      if (order.status !== 'open') return fail(res, 409, 'CONFLICT');
      const input = await readJson(req);
      if (!input || !validItems(input.items)) return fail(res, 400, 'VALIDATION');
    ...
    if (req.method === 'DELETE') {
      if (user.role !== 'admin' && order.owner !== user.id) return fail(res, 403, 'FORBIDDEN'); // they can see it, but may not delete it
